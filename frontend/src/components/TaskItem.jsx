...
const API_BASE = 'https://taskflowbackend-ufhy.onrender.com/api/tasks'
...
const TaskItem = ({ task, onRefresh, onLogout, showCompleteCheckbox = true }) => {
  ...
  const handleComplete = async () => {
    ...
    try {
      await axios.put(`${API_BASE}/${task._id || task.id}/gp`, { completed: newStatus },
            ...
            {showMenu && (
              <div className={TI_CLASSES.menuDropdown}>
                {MENU_OPTIONS.map(opt => (
                  <button role="menuitem" key={opt.action} onClick={() => handleAction(opt.action)} className=' w-full px-3 sm:px-4 py-2 text-left text-xs sm:text-sm hover:bg-purple-50 flex items-center gap-2 transition-colors duration-200 '>
                    {opt.icon}{opt.label}
                  </button>
                ))}
              </div>
